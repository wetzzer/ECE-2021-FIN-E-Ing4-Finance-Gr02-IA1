    ...
    public class TechniquesHumainesSolver : ISudokuSolver
    {
        public void Solve(GrilleSudoku s)
        {
            ...
            else
            {
                ...
                var boardArray2 = board.Select(r => r.ToArray()).ToArray();
                var puzzle2 = new Puzzle(boardArray, false);

                List allCells = null;
                Stack exploredCellValues = null;

                puzzle2.RefreshCandidates();
                do
                {
                    deadEnd = false;
                    ...
                    full = puzzle2.Rows.All(row => row.GetCellsWithCandidate(0).All(c => c.Value != 0));
                    ...
                    if (!full)
                    {
                        ...
                        if (allCells == null)
                        {
                            allCells = puzzle2.Rows.SelectMany((row, rowIdx) => row.GetCellsWithCandidate(0)).ToList();
                        ...
                        if (minCandidates != int.MaxValue)
                        {
                            ...
                            {
                                currentlyExploredCellValues = new BackTrackingState() { Board = puzzle2.GetBoard(), Cell = targetCell, ExploredValues = new List() };
                    ...
                    if (deadEnd)
                    {
                        ...
                        currentlyExploredCellValues.Backtrack(puzzle2);
