...
namespace Sudoku.TechniquesHumaines
{
    public class TechniquesHumainesSolver : ISudokuSolver
    {
        public void Solve(GrilleSudoku s)
        {
            ...
            solver.DoExplorationWork(puzzle);
            foreach (var i in Enumerable.Range(0, 9))
            {
    ...
    internal sealed class Solver
    {
        ...
        public void DoExplorationWork(Puzzle myPuzzle)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    // Solved or failed to solve
                    if (full || (!changed && !RunTechnique()))
                    {
                        break;
                    }
                } while (true);

                full = myPuzzle.Rows.All(row => row._cells.All(c => c.Value != 0));
                ...
                if (!full)
                {
                    ...
                    if (allCells == null)
                    {
                        allCells = myPuzzle.Rows.SelectMany((row, rowIdx) => row._cells).ToList();
                    ...
                    if (minCandidates != int.MaxValue)
                    {
                        ...
                        if (exploredCellValues.Count == 0 || !exploredCellValues.Peek().Equals(targetCell))
                        {
                            currentlyExploredCellValues = new BackTrackingState() { Board = myPuzzle.GetBoard(myPuzzle), Cell = targetCell, ExploredValues = new List<int>() };
                            exploredCellValues.Push(currentlyExploredCellValues);
                        }
                        else
                        {
                            currentlyExploredCellValues = (BackTrackingState)exploredCellValues.Peek();
                        }
                    ...
                    else
                    {
                        ...
                        foreach (var i in Enumerable.Range(0, 9))
                        {
                            foreach(var j in Enumerable.Range(0, 9))
                            {
                                cells.Add(myPuzzle.GetBoard(myPuzzle)[i][j]);
                            }
                        }
                        GrilleSudoku soduk = new GrilleSudoku(cells);
                        //Plus de candidats possibles, on atteint un cul-de-sac
                        if (soduk.IsValid(soduk))
                        {
                            solved = true;
                        }
                        else
                ...
                if (deadEnd)
                {
                    //On se retrouve bloqué, il faut gommer et tenter d'autres hypo thèses
                    BackTrackingState currentlyExploredCellValues = (BackTrackingState)exploredCellValues.Peek();
                    //On annule la dernière assignation
                    currentlyExploredCellValues.Backtrack(myPuzzle);
